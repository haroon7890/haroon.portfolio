    ...
    <main className="section" id="recruiter">
      <div className="glass glass-hover max-w-3xl mx-auto bg-gradient-to-br from-[rgba(255,253,249,0.8)] via-[rgba(255,250,245,0.75)] to-[rgba(255,248,240,0.8)] rounded-3xl p-8 shadow-lg backdrop-blur-md border border-[color:var(--glass-border)] hover:border-[color:var(--accent)]/30">
        ...
        <ul className="mt-6 space-y-3">
          ...
          <li className="body-copy"><strong>Response time:</strong> {SITE_CONFIG.responseTime} (same day guaranteed).</li>
          <li className="body-copy"><strong>Top skills:</strong> Full-stack development, Next.js, Node.js, system design, database optimization, OOP patterns.</li>
          <li className="body-copy"><strong>Selected outcomes:</strong> 90% reduction in demo setup for dispatch systems; 35% improvement in operational efficiency for enterprise platforms.</li>
          <li className="body-copy"><strong>Interview-ready:</strong> Can provide code samples, case-study walkthroughs, live demos, and client references.</li>
        </ul>
            ...
            {SITE_CONFIG.skills.slice(0, 6).map((skill) => (
              <div key={skill.name} className="glass glass-hover rounded-lg backdrop-blur-sm border border-[color:var(--glass-border)] bg-[rgba(255,255,255,0.6)] px-3 py-2 transition-all hover:bg-[rgba(255,255,255,0.8)]">
                ...
                <div className="flex items-center justify-between mt-1">
                  <span className="text-xs text-white font-semibold bg-gradient-to-r from-[#06b6d4] to-[#0891b2] px-2 py-1 rounded">{skill.level}</span>
        ...
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="glass glass-hover rounded-lg backdrop-blur-sm border border-[color:var(--glass-border)] bg-gradient-to-br from-[rgba(37,99,235,0.12)] to-[rgba(59,130,246,0.08)] p-4 shadow-sm">
            ...
            <div className="mt-3 flex flex-col gap-2">
              ...
              <a href={`mailto:${SITE_CONFIG.email}?subject=Let's%20Talk`} className="rounded-md border border-[color:var(--accent)] bg-white px-3 py-2 text-center text-[color:var(--accent)] hover:bg-cyan-50 transition-colors shadow-sm hover:shadow-md">Email Direct</a>
            </div>
          </div>
          <div className="glass glass-hover rounded-lg backdrop-blur-sm border border-[color:var(--glass-border)] bg-gradient-to-br from-[rgba(37,99,235,0.12)] to-[rgba(59,130,246,0.08)] p-4 shadow-sm">
            <h4 className="font-semibold text-[color:var(--accent)]">Learn More</h4>
            <div className="mt-3 flex flex-col gap-2">
              <Link href="/onepager" className="rounded-md border border-[color:var(--border)] bg-white px-3 py-2 text-center hover:bg-cyan-50 transition-colors shadow-sm hover:shadow-md">One-Page Summary</Link>
              <a href={SITE_CONFIG.github} target="_blank" rel="noreferrer" className="rounded-md border border-[color:var(--border)] bg-white px-3 py-2 text-center hover:bg-cyan-50 transition-colors shadow-sm hover:shadow-md">GitHub Profile</a>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 border-t border-[color:var(--border)] pt-6">
          ...
          <ul className="mt-3 space-y-2 text-sm text-[color:var(--text-dim)]">
            <li>• <strong>GitHub:</strong> <a href={SITE_CONFIG.github} target="_blank" rel="noreferrer" className="text-[color:var(--accent)] hover:underline">haroon7890</a> — 2 featured projects</li>
            ...
            {SITE_CONFIG.upwork && (
              <li>• <strong>Upwork:</strong> <a href={SITE_CONFIG.upwork} target="_blank" rel="noreferrer" className="text-[color:var(--accent)] hover:underline">View Profile</a> — 5/5 rated</li>
            )}
          </ul>
        </div>

        <div className="mt-8 rounded-lg bg-cyan-50 border border-cyan-200 p-6">
          ...
          {SITE_CONFIG.calendlyUrl && (
            <a 
              ...
              target="_blank" 
              rel="noreferrer"
              className="mt-4 inline-block rounded-md bg-[color:var(--accent)] px-4 py-2 text-white text-sm font-semibold hover:brightness-110 transition-all"
            >
